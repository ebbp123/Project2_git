...
{
	int nbytes;
	setup_connection("server.c"); // Specifies the connection point as argument.
  ...
  //get_connection("jim", pipe_SERVER_reading_from_child, pipe_SERVER_writing_to_child);
	//
    ...
    close(pipe_SERVER_reading_from_child[1]); // Closing writing end
    close(pipe_SERVER_writing_to_child[0]); // Closing reading end

    //read(pipe_SERVER_reading_from_child[0], buf, MAX_MSG);
    //fprintf (stdout, "%s\n", buf);
		// Check max user and same user id
    usleep(6000000);
    if (get_connection("client", pipe_SERVER_reading_from_child, pipe_SERVER_writing_to_child) == -1) {
